            ...
            (let [{table-items :table-items} (-> (first ast) :table-items)]
                (if (= (count table-items) 1)
                    (if (Strings/isNullOrEmpty (-> (first table-items) :schema_name))
                        (let [id (.incrementAndGet (.atomicSequence ignite "my_select_views" 0 true)) schema_name (-> (first table-items) :schema_name) table_name (-> (first table-items) :table_name) data_set_id (get-data-set-id-by-group-id ignite group_id)]
                            (MyCacheExUtil/transCache ignite [(MyNoSqlCache. "my_select_views" schema_name table_name id (MySelectViews. id table_name data_set_id code) (SqlType/INSERT)) (MyNoSqlCache. "my_select_view_ast" schema_name table_name (MyViewAstPK. schema_name table_name group_id) (my-select-plus/sql-to-ast (my-lexical/to-back code)) (SqlType/INSERT))]))
                        (let [id (.incrementAndGet (.atomicSequence ignite "my_select_views" 0 true)) schema_name (-> (first table-items) :schema_name) table_name (-> (first table-items) :table_name) data_set_id (get-data-set-id-by-ds-name ignite (-> (first table-items) :schema_name))]
                            (MyCacheExUtil/transCache ignite [(MyNoSqlCache. "my_select_views" schema_name table_name id (MySelectViews. id table_name data_set_id code) (SqlType/INSERT)) (MyNoSqlCache. "my_select_view_ast" schema_name table_name (MyViewAstPK. schema_name table_name group_id) (my-select-plus/sql-to-ast (my-lexical/to-back code)) (SqlType/INSERT))]))
                        )
                    )))))
...
(defn smart-view-insert [^Ignite ignite ^Long group_id lst code]
    (let [{schema_name :schema_name table_name :table_name vs-line :vs-line} (my-insert/insert-body (rest (rest lst)))]
        (if (Strings/isNullOrEmpty schema_name)
            (let [id (.incrementAndGet (.atomicSequence ignite "my_insert_views" 0 true)) data_set_id (get-data-set-id-by-group-id ignite group_id)]
                (MyCacheExUtil/transCache ignite [(MyNoSqlCache. "my_insert_views" schema_name table_name id (MyInsertViews. id table_name data_set_id code) (SqlType/INSERT)) (MyNoSqlCache. "my_insert_view_ast" schema_name table_name (MyViewAstPK. schema_name table_name group_id) (my-lexical/to-back code) (SqlType/INSERT))]))
            (let [id (.incrementAndGet (.atomicSequence ignite "my_insert_views" 0 true)) data_set_id (get-data-set-id-by-ds-name ignite schema_name)]
                (MyCacheExUtil/transCache ignite [(MyNoSqlCache. "my_insert_views" schema_name table_name id (MyInsertViews. id table_name data_set_id code) (SqlType/INSERT)) (MyNoSqlCache. "my_insert_view_ast" schema_name table_name (MyViewAstPK. schema_name table_name group_id) (my-lexical/to-back code) (SqlType/INSERT))]))
            )))
...
(defn smart-view-update [^Ignite ignite ^Long group_id lst code]
    (let [{schema_name :schema_name table_name :table_name vs-line :vs-line} (my-update/get_table_name lst)]
        (if (Strings/isNullOrEmpty schema_name)
            (let [id (.incrementAndGet (.atomicSequence ignite "my_update_views" 0 true)) data_set_id (get-data-set-id-by-group-id ignite group_id)]
                (MyCacheExUtil/transCache ignite [(MyNoSqlCache. "my_update_views" schema_name table_name id (MyUpdateViews. id table_name data_set_id code) (SqlType/INSERT)) (MyNoSqlCache. "my_update_view_ast" schema_name table_name (MyViewAstPK. schema_name table_name group_id) (my-lexical/to-back code) (SqlType/INSERT))]))
            (let [id (.incrementAndGet (.atomicSequence ignite "my_update_views" 0 true)) data_set_id (get-data-set-id-by-ds-name ignite schema_name)]
                (MyCacheExUtil/transCache ignite [(MyNoSqlCache. "my_update_views" schema_name table_name id (MyUpdateViews. id table_name data_set_id code) (SqlType/INSERT)) (MyNoSqlCache. "my_update_view_ast" schema_name table_name (MyViewAstPK. schema_name table_name group_id) (my-lexical/to-back code) (SqlType/INSERT))]))
            )))
...
(defn smart-view-delete [^Ignite ignite ^Long group_id lst code]
    (let [{schema_name :schema_name table_name :table_name where_lst :where_lst} (my-delete/get_table_name lst)]
        (if (Strings/isNullOrEmpty schema_name)
            (let [id (.incrementAndGet (.atomicSequence ignite "my_delete_views" 0 true)) data_set_id (get-data-set-id-by-group-id ignite group_id)]
                (MyCacheExUtil/transCache ignite [(MyNoSqlCache. "my_delete_views" schema_name table_name id (MyDeleteViews. id table_name data_set_id code) (SqlType/INSERT)) (MyNoSqlCache. "my_delete_view_ast" schema_name table_name (MyViewAstPK. schema_name table_name group_id) (my-lexical/to-back code) (SqlType/INSERT))]))
            (let [id (.incrementAndGet (.atomicSequence ignite "my_delete_views" 0 true)) data_set_id (get-data-set-id-by-ds-name ignite schema_name)]
                (MyCacheExUtil/transCache ignite [(MyNoSqlCache. "my_delete_views" schema_name table_name id (MyDeleteViews. id table_name data_set_id code) (SqlType/INSERT)) (MyNoSqlCache. "my_delete_view_ast" schema_name table_name (MyViewAstPK. schema_name table_name group_id) (my-lexical/to-back code) (SqlType/INSERT))]))
            )))
