...
(defn parenthesis-to-clj [ignite group_id m args-dic]
    (cond (> (count (filter #(and (map? %) (contains? % :comma_symbol)) (-> m :parenthesis))) 0) (loop [[f & r] (filter #(and (map? %) (contains? % :comma_symbol)) (-> m :parenthesis)) lst []]
          ...
          (and (> (count (filter #(and (map? %) (contains? % :comparison_symbol)) (-> m :parenthesis))) 0) (= (count (-> m :parenthesis)) 3)) (if (= (-> (second (-> m :parenthesis)) :comparison_symbol) "==")
                                                                                                                                                  (format "(= (my-lexical/get-value %s) (my-lexical/get-value %s))" (token-to-clj ignite group_id (first (-> m :parenthesis)) args-dic) (token-to-clj ignite group_id (last (-> m :parenthesis)) args-dic))
                                                                                                                                                  (format "(%s (my-lexical/get-value %s) (my-lexical/get-value %s))" (-> (second (-> m :parenthesis)) :comparison_symbol) (token-to-clj ignite group_id (first (-> m :parenthesis)) args-dic) (token-to-clj ignite group_id (last (-> m :parenthesis)) args-dic)))
...
(defn token-lst-to-clj [ignite group_id m args-dic]
    (cond (and (= (count m) 3) (and (contains? (second m) :comparison_symbol) (= (-> (second m) :comparison_symbol) "=")) (contains? (first m) :item_name)) (format "(.setVar %s %s)" (-> (first m) :item_name) (token-to-clj ignite group_id (last m) args-dic))
          ...
          (and (= (count m) 3) (and (contains? (second m) :comparison_symbol) (= "==" (-> (second m) :comparison_symbol))) (contains? (first m) :item_name)) (format "(= %s %s)" (token-to-clj ignite group_id (first m) args-dic) (token-to-clj ignite group_id (last m) args-dic))
